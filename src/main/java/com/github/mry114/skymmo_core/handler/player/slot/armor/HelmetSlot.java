...
import com.github.mry114.skymmo_core.api.item.content.diff.capa.ICustomItemRequirement;
...
public class HelmetSlot implements IEquipmentSlot {
    @Override
    ...
    public boolean canEquipped(IEquipmentItem item, Player player) {
        if (item instanceof ICustomItemRequirement requirement) {
            return item.getItemType() == ArmorType.HELMET && requirement.getRequirement().getItemCanUse(player);
        }
        else return false;
    }
}
